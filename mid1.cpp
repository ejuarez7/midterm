...
{
	double miles = 1.0, tolerance = 0.0;
	if (argc > 1) {
		miles = atof(argv[1]);
	}
...
double test = 0.0,count = 0.1;
int error_count = 0;

cout << "Miles " << "Kilometers " << "KM (acd. table) " << "Difference" << "Error? "<< endl << endl;

for(int i = 0; i < 200; i++){
    //test = arr[i] - milesToKm(i/10);
    cout << count << "   ";
    cout << milesToKm(count*1.0) << "    ";
    cout << arr[i] << "    ";
    test = arr[i] - milesToKm(count);
    cout << test;
    
    /*cout << count+0.1;
    cout << milesToKm(count+0.1) << endl;
    cout << arr[i] << endl;
    test = arr[i] - milesToKm(count+0.1);
    cout << test;*/
